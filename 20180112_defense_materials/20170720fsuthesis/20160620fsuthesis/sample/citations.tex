...
%%% new macro so that I can easily type the BibTeX logo.
...
bibliography.  There are two options: (1)~use the \lit{references}
...
discipline-specific style guide, see \citet{gopikrishnan2000statistical}
or \citet{greenwade93} for lots of bibliographic examples.)
...
\section{Citations and B{\small IB}\TeX}
...
\subsection{The \pkg{natbib} Package}
...
compatibility with the popular \pkg{natbib} package.  With this
compatibility mode enabled, you may use \pkg{natbib}-style citation
...
newer system called \pkg{biblatex}.  Bibliography styles based on this
system use different citation commands and generate bibliographies
in a different way.

For a comparison of the \BibTeX{} and \pkg{biblatex} systems, the
links below provide useful information.  To use the \pkg{biblatex}
...
While the \pkg{natbib} package provides an optional bibliography style
that closely follows the so-called ``Chicago style''
(see \cite{Anonymous:1993:CMS}), the \pkg{biblatex-chicago} package
...
called \lit{biber} rather than the \BibTeX{} program.  The
major \TeX{} distributions already include the required programs.
However, you may need to configure your environment to run \lit{biber}
rather than \BibTeX{} to process your document.

\subsection{The \pkg{biblatex-historian} Package}
A \pkg{historian} package is similar to \pkg{biblatex-chicago}, but it
...
This package also requires the \lit{biber} back-end processor.
