...
\begin{center}
   	...
   	\begin{table}[H]
   		\begin{tabular}{|l|l| }
   			...
   			$v_1=\{P_i^{ask},  V_i^{ask},  P_i^{bid},  V_i^{bid}\}_{i=1}^n$  (40)   & price\ and\ volume(n levels)\\[5pt]
   			\hline
            Time-insensitive Set & Description(i=level index)\\[5pt]
   			\hline 
   			$v_2=\{(P_i^{ask}-P_i^{bid}),  (P_i^{ask}+P_i^{bid})/2\}_{i=1}^n$  (20)   & price\ and\ volume(n levels)\\[5pt]
   			
   			$v_3=\{(P_i^{ask}-P_1^{ask}),  (P_1^{bid}+P_i^{bid}),  |P_{i+1}^{ask}-P_i^{ask}|,  |P_{i+1}^{bid}-P_i^{bid}|\}_{i=1}^n$  (36) & price difference\\[5pt]
   			
   			$v_4=\{\frac{1}{n}\sum_{i=1}^n P_i^{ask},  \frac{1}{n}\sum_{i=1}^n P_i^{bid},  \frac{1}{n}\sum_{i=1}^n V_i^{ask},  \frac{1}{n}\sum_{i=1}^n V_i^{bid} \}$  (4)& mean prices and volumes\\[5pt]
   			
   			$v_5=\{\sum_{i=1}^{n}(P_i^{ask}-P_i^{bid}),  \sum_{i=1}^{n}(V_i^{ask}-V_i^{bid})\}$  (2)   & accumulated difference\\[5pt]
...
\begin{table}[h!]\scriptsize
  \caption{Binary prediction results of stock AMZN }
\begin{center}
\scalebox{0.8}{
...
\end{block}
\scriptsize{Remark: \alert{training samples 90000 and test samples 10000}. The estimation number for AdaBoost and random forest is 100.Computer is 8G memory and Intel Xeon E3 processor(4 cores)}
\end{frame}​
